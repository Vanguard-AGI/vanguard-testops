...
import io.vanguard.testops.validation.groups.*;
...
public class FunctionalCase implements Serializable {
    ...
    @Schema(description = "来源用例ID（复制或从两库导入时记录被复制用例的id），用于执行归父、复用统计")
    @Size(max = 50, groups = {Created.class, Updated.class})
    private String sourceCaseId;

    @Schema(description = "复用类型：DIRECT_REUSE(直接复用-仅改标题)/ADAPT_REUSE(适配复用-改了其它)，仅当来源两库时有效")
    ...
    public enum Column {
        ...
        sourceCaseId("source_case_id", "sourceCaseId", "VARCHAR", false),
        reuseType("reuse_type", "reuseType", "VARCHAR", false),
