...
import io.vanguard.testops.api.dto.request.http.body.BodyParamType;
import io.vanguard.testops.api.dto.request.http.body.FormDataKV;
import io.vanguard.testops.api.dto.request.http.body.WWWFormKV;
import io.vanguard.testops.api.support.parser.postman.PostmanItem;
import io.vanguard.testops.api.support.parser.postman.PostmanKeyValue;
import io.vanguard.testops.api.support.parser.postman.PostmanRequest;
import io.vanguard.testops.api.support.parser.postman.PostmanResponse;
...
public abstract class PostmanAbstractApiDefinitionImportParser<T> extends HttpApiDefinitionImportAbstractParser<T> {
    ...
    private MsHTTPElement parseElement(PostmanRequest requestDesc, String name, String url, JsonNode urlNode) {
        ...
        if (urlNode instanceof ObjectNode) {
            ObjectNode urlObject = (ObjectNode) urlNode;
            //设置query参数
    ...
    private static void setRaw(JsonNode bodyNode, MsHTTPElement request) {
        ...
        if (ObjectUtils.allNotNull(rawNode, optionNode)) {
            if (optionNode instanceof ObjectNode) {
                ObjectNode optionObject = (ObjectNode) optionNode;
                JsonNode languageNode = optionObject.get(RAW).get(La);
                if (languageNode instanceof TextNode) {
                    TextNode languageText = (TextNode) languageNode;
    ...
    private static void setWwwData(JsonNode bodyNode, MsHTTPElement request) {
        ...
        if (modeNode instanceof ArrayNode) {
            ArrayNode urlEncodedArray = (ArrayNode) modeNode;
    ...
    private static void setFormData(JsonNode bodyNode, MsHTTPElement request) {
        ...
        if (modeNode instanceof ArrayNode) {
            ArrayNode formArray = (ArrayNode) modeNode;
            formArray.forEach(f -> {
    ...
    private static void setRest(JsonNode urlObject, MsHTTPElement request) {
        ...
        if (restNode instanceof ArrayNode) {
            ArrayNode restArray = (ArrayNode) restNode;
            restArray.forEach(r -> {
    ...
    private static void setQuery(JsonNode urlObject, MsHTTPElement request) {
        ...
        if (queryNode instanceof ArrayNode) {
            ArrayNode queryArray = (ArrayNode) queryNode;
    ...
    private static String getUrl(JsonNode urlNode) {
            ...
            if (pathNode instanceof ArrayNode) {
                ...
                pathArray.forEach(p -> {
                    if (p instanceof TextNode) {
                        TextNode pathString = (TextNode) p;
        ...
        } else if (urlNode instanceof TextNode) {
            url = ((TextNode) urlNode).asText();
        }
        return url;
    }
    ...
    private static DigestAuth buildAuth(String type, JsonNode authNode) {
        ...
        if (digestNode != null) {
            if (digestNode instanceof ObjectNode) {
                ObjectNode digestObject = (ObjectNode) digestNode;
            ...
            } else if (digestNode instanceof ArrayNode) {
                ArrayNode digestArray = (ArrayNode) digestNode;
