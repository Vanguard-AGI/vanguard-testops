...
import io.vanguard.testops.api.dto.converter.ApiDefinitionImportFileParseResult;
...
import io.vanguard.testops.api.dto.request.http.MsHeader;
import io.vanguard.testops.api.dto.request.http.QueryParam;
...
import io.vanguard.testops.api.support.schema.JsonSchemaBuilder;
import io.vanguard.testops.api.support.format.xml.XMLUtil;
import io.vanguard.testops.plugin.api.spi.AbstractMsTestElement;
import io.vanguard.testops.project.constants.PropertyConstant;
import io.vanguard.testops.project.dto.environment.auth.NoAuth;
...
public class Swagger3ApiDefinitionImportParser extends HttpApiDefinitionImportAbstractParser<ApiDefinitionImportFileParseResult> {
    ...
    private List<AuthorizationValue> setAuths(ImportRequest request) {
        ...
        if (request.isAuthSwitch() && (StringUtils.isNotBlank(request.getAuthUsername()) || StringUtils.isNotBlank(request.getAuthPassword()))) {
            AuthorizationValue authorizationValue = new AuthorizationValue();
            authorizationValue.setType(HEADER);
            authorizationValue.setKeyName("Authorization");
            String credentials = StringUtils.defaultString(request.getAuthUsername()) + ":" + StringUtils.defaultString(request.getAuthPassword());
        ...
        if (StringUtils.isNotBlank(request.getSwaggerToken())) {
            String[] tokenRows = StringUtils.split(request.getSwaggerToken(), '\n');
            for (String row : tokenRows) {
                String normalizedRow = StringUtils.trimToEmpty(row);
                if (StringUtils.isBlank(normalizedRow)) {
                    continue;
                }
                int separatorIndex = normalizedRow.indexOf(':');
                if (separatorIndex <= 0) {
                    continue;
                }
                String key = StringUtils.trimToEmpty(normalizedRow.substring(0, separatorIndex));
                String value = StringUtils.trimToEmpty(normalizedRow.substring(separatorIndex + 1));
                if (StringUtils.isBlank(key)) {
                    continue;
                }
                AuthorizationValue authorizationValue = new AuthorizationValue();
                authorizationValue.setType(HEADER);
                authorizationValue.setKeyName(key);
                authorizationValue.setValue(value);
                auths.add(authorizationValue);
            }
        }
    ...
    private JsonSchemaItem parseSchema(Schema<?> schema, Set refModelSet) {
        if (schema != null) {
            ...
            if (schema instanceof ArraySchema) {
                return parseArraySchema((ArraySchema) schema, refModelSet);
            } else if (schema instanceof ObjectSchema) {
                return parseObject((ObjectSchema) schema, refModelSet);
            } else if (schema instanceof MapSchema) {
                return parseMapObject((MapSchema) schema, refModelSet);
            } else if (schema instanceof IntegerSchema) {
                return parseInteger((IntegerSchema) schema);
            } else if (schema instanceof StringSchema) {
                return parseString((StringSchema) schema);
            } else if (schema instanceof NumberSchema) {
                return parseNumber((NumberSchema) schema);
            } else if (schema instanceof BooleanSchema) {
                return parseBoolean((BooleanSchema) schema);
            } else if (StringUtils.isNotBlank(schema.get$ref())) {
                return parseObject(schema, refModelSet);
            } else {
                return parseSchemaByType(schema, refModelSet);
            }
        }
        return null;
    }
    ...
    private JsonSchemaItem parseSchemaByType(Schema<?> schema, Set refModelSet) {
        ...
        switch (type) {
            ...
            default:
                return createDefaultJsonSchemaItem(schema);
        }
    }
