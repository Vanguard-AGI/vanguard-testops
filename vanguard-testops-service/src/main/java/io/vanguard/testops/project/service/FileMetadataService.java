...
import io.vanguard.testops.project.dto.filemanagement.FileManagementQuery;
import io.vanguard.testops.project.dto.filemanagement.request.*;
import io.vanguard.testops.project.dto.filemanagement.response.FileInformationResponse;
import io.vanguard.testops.project.dto.filemanagement.response.FileVersionResponse;
import io.vanguard.testops.project.mapper.ExtFileMetadataMapper;
import io.vanguard.testops.project.mapper.FileMetadataMapper;
import io.vanguard.testops.project.mapper.FileMetadataRepositoryMapper;
import io.vanguard.testops.project.mapper.FileModuleRepositoryMapper;
import io.vanguard.testops.project.support.file.FileDownloadUtils;
import io.vanguard.testops.project.support.file.FileMetadataUtils;
...
import io.vanguard.testops.sdk.dto.RemoteFileAttachInfo;
...
public class FileMetadataService {
    ...
    public FileMetadata genFileMetadata(String fileSpecifyName, String filePath, String storage, long size, boolean enable, String projectId, String moduleId, String operator) {
        ...
        //检查处理后的用户名合法性
        if (StringUtils.equals(storage, StorageType.OSS.name())) {
    ...
    public String upload(FileUploadRequest request, String operator, MultipartFile uploadFile) throws Exception {
        ...
        FileMetadata fileMetadata = this.genFileMetadata(null, fileName, StorageType.OSS.name(), uploadFile.getSize(), request.isEnable(), request.getProjectId(), request.getModuleId(), operator);
    ...
    public String transferFile(String fileName, String originFileName, String projectId, String moduleId, String operator, byte[] fileBytes) {
        ...
        FileMetadata fileMetadata = this.genFileMetadata(fileName, originFileName, StorageType.OSS.name(), fileBytes.length, false, projectId, moduleId, operator);
        ...
        uploadFileRequest.setStorage(StorageType.OSS.name());

        FileRepository minio = CommonBeanFactory.getBean(OssRepository.class);
        String filePath;
        try {
    ...
    private void checkMinIOFileName(String id, String fileName, String type, String projectId) {
        ...
        if (StringUtils.isBlank(id)) {
            example.createCriteria().andNameEqualTo(fileName).andTypeEqualTo(type).andLatestEqualTo(true).andProjectIdEqualTo(projectId).andStorageEqualTo(StorageType.OSS.name());
        } else {
            example.createCriteria().andNameEqualTo(fileName).andTypeEqualTo(type).andLatestEqualTo(true).andProjectIdEqualTo(projectId).andIdNotEqualTo(id).andStorageEqualTo(StorageType.OSS.name());
        }
    ...
    public String reUpload(FileReUploadRequest request, String operator, MultipartFile uploadFile) throws Exception {
        ...
        //非OSS类型文件不允许重新上传
        if (!StringUtils.equals(oldFile.getStorage(), StorageType.OSS.name())) {
            throw new MSException(Translator.get("file.not.exist"));
        }
        //旧文件latest修改
