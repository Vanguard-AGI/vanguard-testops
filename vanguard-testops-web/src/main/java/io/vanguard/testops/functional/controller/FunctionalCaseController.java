...
import io.vanguard.testops.functional.dto.FunctionalCaseVersionDTO;
import io.vanguard.testops.functional.dto.response.FunctionalCaseImportResponse;
import io.vanguard.testops.functional.support.importexport.excel.domain.FunctionalCaseExportColumns;
import io.vanguard.testops.functional.request.*;
import io.vanguard.testops.functional.service.*;
import io.vanguard.testops.project.dto.CustomFieldOptions;
...
public class FunctionalCaseController {
    ...
    @Operation(summary = "用例管理-功能用例-模块层级复制（复制模块及其所有子模块下的用例）")
    @RequiresPermissions(PermissionConstants.FUNCTIONAL_CASE_READ_UPDATE)
    @CheckOwner(resourceId = "#request.getProjectId()", resourceType = "project")
    public void copyModuleWithCases(@Validated @RequestBody FunctionalCaseModuleCopyRequest request) {
        String userId = SessionUtils.getUserId();
        String organizationId = SessionUtils.getCurrentOrganizationId();
        functionalCaseService.copyModuleWithCases(request.getSourceModuleId(), request.getTargetModuleId(), request.getProjectId(), userId, organizationId);
    }
