...
import io.vanguard.testops.api.dto.*;
...
import io.vanguard.testops.api.dto.scenario.ApiFileCopyRequest;
...
import io.vanguard.testops.api.support.diff.HttpRequestParamDiffUtils;
...
import io.vanguard.testops.sdk.constants.ApiFileResourceType;
...
public class ApiTestCaseService extends MoveNodeService {
    ...
    public void deleteResourceByIds(List<String> ids, String projectId, String userId) {
        ...
        // 删除报告与用例的直接关联记录，但保留报告主数据
        ApiTestCaseRecordExample recordExample = new ApiTestCaseRecordExample();
        recordExample.createCriteria().andApiTestCaseIdIn(ids);
        apiTestCaseRecordMapper.deleteByExample(recordExample);

        // 删除测试计划与 API 用例的直接关联，但保留测试计划历史报告数据
        TestPlanApiCaseExample testPlanApiCaseExample = new TestPlanApiCaseExample();
        testPlanApiCaseExample.createCriteria().andApiCaseIdIn(ids);
        ...
        // TODO 如果后续接入通用附件关联表，需要在这里补齐附件关系删除；报告主数据不删除
