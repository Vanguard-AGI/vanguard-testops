...
CREATE TABLE IF NOT EXISTS bug(
    ...
    `handle_users` VARCHAR(1000) NULL COMMENT '处理人集合;预留字段, 后续工作台统计可能需要',
    ...
    `platform_bug_id` VARCHAR(50) NULL COMMENT '第三方平台缺陷ID',
    `delete_user` VARCHAR(50) NULL COMMENT '删除人',
    `delete_time` BIGINT NULL COMMENT '删除时间',
    `deleted` BIT(1) NOT NULL COMMENT '删除状态',
    `pos` BIGINT NOT NULL COMMENT '自定义排序，间隔5000',
    `feishu_story_id` VARCHAR(100) NULL COMMENT '关联的飞书需求(Story)ID',
    `defect_type` VARCHAR(50) NULL COMMENT '缺陷类型(功能性缺陷/性能缺陷/界面缺陷等)',
    `defect_reason` VARCHAR(255) NULL COMMENT '缺陷原因',
    `app_id` VARCHAR(50) NULL COMMENT '所属应用',
    `affected_app_ids` VARCHAR(500) NULL COMMENT '关联影响应用(多选)',
    `discovery_phase` VARCHAR(50) NULL COMMENT '发现阶段',
    `business_line` VARCHAR(50) NULL COMMENT '业务线',
    `discovery_difficulty` VARCHAR(50) NULL COMMENT '缺陷发现难易度',
    `reopen_count` INT NOT NULL DEFAULT 0 COMMENT '重开次数',
    `discoverer` VARCHAR(100) NULL COMMENT '发现人',
    `actual_time` BIGINT NULL COMMENT '实际时间(毫秒时间戳)',
    PRIMARY KEY (id)
...
CREATE INDEX idx_project_id_delete_create_time ON bug(project_id, deleted, create_time);
CREATE INDEX idx_project_id_delete_create_time_create_user ON bug(project_id, deleted, create_time, create_user);
