...
import io.vanguard.testops.api.runtime.config.KafkaConfig;
...
import io.vanguard.testops.api.dto.debug.ApiDebugRunRequest;
import io.vanguard.testops.api.dto.debug.ApiResourceRunRequest;
import io.vanguard.testops.api.dto.definition.ApiModuleDTO;
import io.vanguard.testops.api.dto.definition.EnvApiModuleRequest;
import io.vanguard.testops.api.dto.definition.EnvApiTreeDTO;
...
import io.vanguard.testops.project.dto.customfunction.request.CustomFunctionRunRequest;
import io.vanguard.testops.project.dto.environment.EnvironmentInfoDTO;
import io.vanguard.testops.project.dto.environment.GlobalParams;
...
import io.vanguard.testops.project.dto.environment.http.HttpConfigModuleMatchRule;
import io.vanguard.testops.project.dto.environment.http.SelectModule;
import io.vanguard.testops.project.service.*;
import io.vanguard.testops.sdk.constants.*;
import io.vanguard.testops.sdk.dto.api.task.*;
import io.vanguard.testops.sdk.exception.IResultCode;
import io.vanguard.testops.sdk.exception.MSException;
import io.vanguard.testops.sdk.exception.TaskRunnerResultCode;
...
import io.vanguard.testops.system.utils.EngineBridge;
...
import static io.vanguard.testops.api.controller.result.ApiResultCode.RESOURCE_POOL_EXECUTE_ERROR;
...
public class ApiExecuteService {
    ...
    private void nodeExecute(TaskRequestDTO taskRequest, TestResourcePoolReturnDTO testResourcePoolDTO) throws Exception {
        ...
        while (true) {
            ...
            try {
                if (ApiExecuteRunMode.isDebug(taskInfo.getRunMode())) {
                    EngineBridge.debugApi(endpoint, taskRequest);
                } else {
                    EngineBridge.runApi(endpoint, taskRequest);
                    // 更新状态项资源池信息
    ...
    private void k8sExecute(TaskRequestDTO taskRequest, TestResourcePoolReturnDTO testResourcePoolDTO) throws Exception {
        ...
        if (ApiExecuteRunMode.isDebug(taskInfo.getRunMode())) {
            EngineBridge.debugApi(taskRequest, testResourceDTO);
        } else {
            try {
                EngineBridge.runApi(taskRequest, testResourceDTO);
            } catch (Exception e) {
                apiCommonService.updateTaskItemErrorMassage(taskItem.getId(), TaskItemErrorMessage.INVALID_RESOURCE_POOL);
                throw e;
            }
        ...
        while (distributeTaskIterator.hasNext()) {
            ...
            while (nodeIterator.hasNext()) {
                ...
                try {
                    ...
                    EngineBridge.batchRunApi(endpoint, subTaskRequest);
    ...
    private void k8sBatchExecute(TaskBatchRequestDTO taskRequest, TaskInfo taskInfo, TestResourcePoolReturnDTO testResourcePool) throws Exception {
        ...
        taskInfo.setPoolSize(testResourceDTO.getConcurrentNumber());
        taskInfo.setPerTaskSize(testResourceDTO.getPodThreads());
        EngineBridge.batchRunApi(taskRequest, testResourceDTO);
    }
    ...
    private void setServerInfoParam(TaskInfo taskInfo) {
        ...
        taskInfo.setMinioConfig(JSON.toJSONString(getOssConfig()));
    ...
    private ApiExecuteFileInfo getApiExecuteFileInfo(String fileId, String fileName, String projectId) {
        return getApiExecuteFileInfo(fileId, fileName, projectId, StorageType.OSS.name());
    }
    ...
    public static Map<String, String> getOssConfig() {
        OssProperties ossProperties = CommonBeanFactory.getBean(OssProperties.class);
        Map<String, String> ossConfig = new HashMap<>();
        assert ossProperties != null;
        ossConfig.put("endpoint", ossProperties.getEndpoint());
        ossConfig.put("accessKey", ossProperties.getAccessKey());
        ossConfig.put("secretKey", ossProperties.getSecretKey());
