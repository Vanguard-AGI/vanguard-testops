...
import io.vanguard.testops.functional.dto.ExportTaskDTO;
import io.vanguard.testops.functional.dto.response.FunctionalCaseImportResponse;
import io.vanguard.testops.functional.support.importexport.excel.constants.FunctionalCaseImportField;
import io.vanguard.testops.functional.support.importexport.excel.converter.FunctionalCaseExportConverter;
import io.vanguard.testops.functional.support.importexport.excel.converter.FunctionalCaseExportConverterFactory;
import io.vanguard.testops.functional.support.importexport.excel.domain.*;
import io.vanguard.testops.functional.support.importexport.excel.handler.FunctionCaseMergeWriteHandler;
import io.vanguard.testops.functional.support.importexport.excel.handler.FunctionCaseTemplateWriteHandler;
import io.vanguard.testops.functional.support.importexport.excel.listener.FunctionalCaseCheckEventListener;
import io.vanguard.testops.functional.support.importexport.excel.listener.FunctionalCaseImportEventListener;
import io.vanguard.testops.functional.support.importexport.excel.listener.FunctionalCasePretreatmentListener;
...
import io.vanguard.testops.functional.mapper.ExtFunctionalCaseCommentMapper;
import io.vanguard.testops.functional.request.FunctionalCaseExportRequest;
import io.vanguard.testops.functional.request.FunctionalCaseImportRequest;
import io.vanguard.testops.functional.support.importexport.xmind.parser.XMindCaseParser;
...
import io.vanguard.testops.sdk.dto.ExportMsgDTO;
...
public class FunctionalCaseFileService {
    ...
    private List<List<Object>> parseExcelData2List(List<List<String>> headListParams, List<FunctionalCaseExcelData> data) {
        ...
        FunctionalCaseImportField[] importFields = FunctionalCaseImportField.values();
        ...
        for (FunctionalCaseExcelData model : data) {
            ...
            for (String head : headList) {
                ...
                for (FunctionalCaseImportField importField : importFields) {
                    if (importField.getFieldLangMap().containsValue(head) && StringUtils.equals("name", importField.getValue()) && model.getHyperLinkName() != null) {
                        fields.add(model.getHyperLinkName());
                        isSystemField = true;
                        break;
                    }
                    if (importField.containsHead(head)) {
                        fields.add(importField.parseExcelDataValue(model));
                        isSystemField = true;
                        break;
                    }
    ...
    public FunctionalCaseImportResponse importXMind(FunctionalCaseImportRequest request, SessionUser user, MultipartFile multipartFile) {
        ...
        try {
            ...
            XMindCaseParser xmindParser = new XMindCaseParser(request, customFields, user, lasePos);
            errList = xmindParser.parse(multipartFile);
            // 仅当真正没有解析到任何用例（新增、更新、已存在均无）时才报「导入内容为空」
            // 不覆盖时已存在的用例会进入 checkSuccessList，不应误报内容为空
            if (CollectionUtils.isEmpty(xmindParser.getList())
                    && CollectionUtils.isEmpty(xmindParser.getUpdateList())
                    && CollectionUtils.isEmpty(xmindParser.getCheckSuccessList())) {
                if (errList == null) {
    ...
    private void buildBaseField(FunctionalCaseExcelData data, FunctionalCase functionalCase, FunctionalCaseBlob functionalCaseBlob, Map<String, String> moduleMap,
        ...
        // P9: 两库行填 source_case_id=用例id，非两库留空（导出再导入复用统计）
        // 模块 path 可能为 "用例模板库" 或 "/用例模板库" 等带前缀形式
        String modulePath = moduleMap.get(functionalCase.getModuleId());
        boolean twoLib = modulePath != null && isTwoLibraryModulePath(modulePath);
        if (twoLib) {
            data.setSourceCaseId(functionalCase.getId());
        } else {
            data.setSourceCaseId(null);
        }
    }

    /**
     * 判断模块 path 是否属于两库（用例模板库 / 回归用例库）
     * path 可能为 "用例模板库" 或 "/用例模板库" 或 "/父/用例模板库" 等形式
     */
    private boolean isTwoLibraryModulePath(String modulePath) {
        if (modulePath == null) {
            return false;
        }
        String template = CaseMetricsConstants.TwoLibraryModuleName.TEMPLATE_LIBRARY;
        String regression = CaseMetricsConstants.TwoLibraryModuleName.REGRESSION_LIBRARY;
        return template.equals(modulePath) || regression.equals(modulePath)
                || modulePath.endsWith("/" + template) || modulePath.endsWith("/" + regression);
    }
    ...
    private List<List<String>> getFunctionalCaseExportHeads(FunctionalCaseExportRequest request) {
        ...
        // P9: 始终带列 source_case_id（隐藏、不可修改），两库行填 id，导入时识别有值则写 functional_case.source_case_id
        headList.add(Arrays.asList("source_case_id"));
        return headList;
    }
    ...
    public ResponseEntity<byte[]> downloadFile(String projectId, String fileId, String userId) {
        ...
        String fileName = "Spotter_Aegis_case_" + project.getName() + "." + tasksFirst.getFileType();
