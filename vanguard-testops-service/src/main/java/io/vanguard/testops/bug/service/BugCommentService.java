...
import io.vanguard.testops.bug.domain.BugCommentExample;
...
import io.vanguard.testops.bug.dto.response.BugCommentNoticeDTO;
import io.vanguard.testops.bug.enums.BugAttachmentSourceType;
import io.vanguard.testops.bug.mapper.BugCommentMapper;
...
public class BugCommentService {
    ...
    public BugComment addComment(BugCommentEditRequest request, String currentUser) {
        ...
        if (StringUtils.equals(request.getEvent(), NoticeConstants.Event.REPLY)) {
            result = addBugCommentAndReplyNotice(request, bugComment, currentUser);
        } else {
            result = addBugCommentAndCommentNotice(request, bugComment, currentUser);
        }
        // 评论同步到飞书（缺陷有 platformBugId 时）
        bugService.syncCommentToFeishu(bug.getId(), result.getContent(), currentUser);
        return result;
    }
