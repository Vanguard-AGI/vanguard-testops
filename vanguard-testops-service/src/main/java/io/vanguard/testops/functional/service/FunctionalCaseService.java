...
import io.vanguard.testops.functional.constants.CaseEvent;
...
import io.vanguard.testops.functional.constants.FunctionalCaseReviewStatus;
...
import io.vanguard.testops.functional.result.CaseManagementResultCode;
import io.vanguard.testops.plan.domain.TestPlanCaseExecuteHistoryExample;
import io.vanguard.testops.plan.domain.TestPlanConfig;
import io.vanguard.testops.plan.domain.TestPlanFunctionalCase;
import io.vanguard.testops.plan.domain.TestPlanFunctionalCaseExample;
import io.vanguard.testops.plan.mapper.TestPlanCaseExecuteHistoryMapper;
import io.vanguard.testops.plan.mapper.TestPlanConfigMapper;
import io.vanguard.testops.plan.mapper.TestPlanFunctionalCaseMapper;
import io.vanguard.testops.project.domain.*;
import io.vanguard.testops.project.dto.ModuleCountDTO;
import io.vanguard.testops.project.mapper.ExtBaseProjectVersionMapper;
...
import io.vanguard.testops.provider.BaseCaseProvider;
...
import io.vanguard.testops.sdk.file.FileCopyRequest;
...
import io.vanguard.testops.system.dto.sdk.*;
...
public class FunctionalCaseService {
    ...
    public FunctionalCase addFunctionalCase(FunctionalCaseAddRequest request, List<MultipartFile> files, String userId, String organizationId) {
        ...
        functionalCaseAttachmentService.uploadOssFile(caseId, request.getProjectId(), request.getCaseDetailFileIds(), userId, CaseFileSourceType.CASE_DETAIL.toString());
        ...
        // 用例创建后，自动计算CS值和相关指标
        try {
            caseCSCalculationService.getOrCalculateCSDetail(caseId);
            LogUtils.info("用例创建后自动计算CS值完成，用例ID: {}", caseId);
        } catch (Exception e) {
            LogUtils.error("用例创建后自动计算CS值失败，用例ID: " + caseId, e);
            // CS值计算失败不影响用例创建流程，只记录日志
        }
    ...
    public FunctionalCaseDetailDTO getFunctionalCaseDetail(String functionalCaseId, String userId, boolean checkDetailCount) {
        ...
        // 获取用例来源（用于前端判断是否启动 ModificationTracker）
        try {
            io.vanguard.testops.functional.domain.CaseMetricsDetail csDetail = 
                caseMetricsDetailMapper.selectByCaseId(functionalCaseId);
            if (csDetail != null && csDetail.getCaseSourceType() != null) {
                functionalCaseDetailDTO.setCaseSourceType(csDetail.getCaseSourceType());
            } else {
                functionalCaseDetailDTO.setCaseSourceType("NEW"); // 默认为新建
            }
        } catch (Exception e) {
            LogUtils.warn("获取用例来源失败: caseId=" + functionalCaseId, e);
            functionalCaseDetailDTO.setCaseSourceType("NEW"); // 默认为新建
        }
    ...
    public FunctionalCase updateFunctionalCase(FunctionalCaseEditRequest request, List<MultipartFile> files, String userId) {
        ...
        FunctionalCase functionalCase = new FunctionalCase();
        BeanUtils.copyBean(functionalCase, request);
        // 若来源用例来自两库，按「当前内容 vs 源用例」判定并写入复用类型
        if (StringUtils.isNotBlank(checked.getSourceCaseId()) && isSourceFromTwoLibraries(checked.getSourceCaseId())) {
            FunctionalCase sourceCase = functionalCaseMapper.selectByPrimaryKey(checked.getSourceCaseId());
            FunctionalCaseBlob sourceBlob = functionalCaseBlobMapper.selectByPrimaryKey(checked.getSourceCaseId());
            if (sourceCase != null) {
                String reuseType = computeReuseType(
                        request.getName(), request.getSteps(), request.getTextDescription(),
                        request.getExpectedResult(), request.getPrerequisite(), request.getDescription(),
                        sourceCase, sourceBlob);
                functionalCase.setReuseType(reuseType);
            }
        }
        ...
        //上传富文本文件
        functionalCaseAttachmentService.uploadOssFile(request.getId(), request.getProjectId(), request.getCaseDetailFileIds(), userId, CaseFileSourceType.CASE_DETAIL.toString());
        ...
        // 用例更新后，自动重新计算CS值
        try {
            caseCSCalculationService.recalculateCaseCS(request.getId());
        } catch (Exception e) {
            LogUtils.error("用例更新后重新计算CS值失败，用例ID: " + request.getId(), e);
            // CS值计算失败不影响用例更新流程，只记录日志
        }

        // 判断变更区域并记录变更日志
        String changeArea = determineChangeArea(checked, functionalCase, functionalCaseBlob, request);
        if (StringUtils.isNotBlank(request.getChangeReason())) {
            CaseChangeLog changeLog = new CaseChangeLog();
            changeLog.setCaseId(request.getId());
            changeLog.setProjectId(request.getProjectId());
            changeLog.setChangeReason(request.getChangeReason());
            changeLog.setChangeArea(changeArea != null ? changeArea : "OTHER");
            changeLog.setOperator(userId);
            changeLog.setCreateTime(System.currentTimeMillis());
            caseChangeLogMapper.insert(changeLog);
            
            // 如果是核心内容变更（TITLE/STEP/EXPECTED），更新 modify_count
            if (isCoreContentChange(changeArea)) {
                updateModifyCount(request.getId());
            }
        }
     ...
     * @param oldBlob 原用例详情
     * @param request 更新请求
     * @return TITLE/STEP/EXPECTED/OTHER
     */
    private String determineChangeArea(FunctionalCase oldCase, FunctionalCase newCase, 
                                       FunctionalCaseBlob oldBlob, FunctionalCaseEditRequest request) {
        // 判断标题是否变更
        if (!StringUtils.equals(oldCase.getName(), newCase.getName())) {
            return "TITLE";
        }
        
        // 判断步骤是否变更
        try {
            String oldSteps = oldBlob != null && oldBlob.getSteps() != null ? 
                new String(oldBlob.getSteps(), StandardCharsets.UTF_8) : "";
            String newSteps = StringUtils.defaultIfBlank(request.getSteps(), "");
            if (!StringUtils.equals(oldSteps, newSteps)) {
                return "STEP";
            }
        } catch (Exception e) {
            LogUtils.warn("判断步骤变更失败", e);
        }
        
        // 判断预期结果是否变更
        try {
            String oldExpected = oldBlob != null && oldBlob.getExpectedResult() != null ? 
                new String(oldBlob.getExpectedResult(), StandardCharsets.UTF_8) : "";
            String newExpected = StringUtils.defaultIfBlank(request.getExpectedResult(), "");
            if (!StringUtils.equals(oldExpected, newExpected)) {
                return "EXPECTED";
            }
        } catch (Exception e) {
            LogUtils.warn("判断预期结果变更失败", e);
        }
    ...
    /**
     * 判断是否是核心内容变更
     * @param changeArea 变更区域
     * @return 是否是核心内容变更
     */
    private boolean isCoreContentChange(String changeArea) {
        return "TITLE".equals(changeArea) || "STEP".equals(changeArea) || "EXPECTED".equals(changeArea);
    }
     ...
     * @param caseId 用例ID
     */
    private void updateModifyCount(String caseId) {
        try {
            CaseMetricsDetail detail = caseMetricsDetailMapper.selectByCaseId(caseId);
            if (detail != null) {
                int currentModifyCount = detail.getModifyCount() != null ? detail.getModifyCount() : 0;
                detail.setModifyCount(currentModifyCount + 1);
                detail.setUpdateTime(System.currentTimeMillis());
                caseMetricsDetailMapper.updateByPrimaryKey(detail);
            }
        } catch (Exception e) {
            LogUtils.warn("更新用例修改次数失败，用例ID: " + caseId, e);
        }
    }

    /**
     * 判断来源用例是否属于两库（用例模板库 / 回归用例库）
     */
    private boolean isSourceFromTwoLibraries(String sourceCaseId) {
        if (StringUtils.isBlank(sourceCaseId)) {
            return false;
        }
        try {
            FunctionalCase source = functionalCaseMapper.selectByPrimaryKey(sourceCaseId);
            if (source == null) {
                return false;
            }
            String moduleName = functionalCaseModuleService.getModuleName(source.getModuleId());
            return CaseMetricsConstants.TwoLibraryModuleName.TEMPLATE_LIBRARY.equals(moduleName)
                    || CaseMetricsConstants.TwoLibraryModuleName.REGRESSION_LIBRARY.equals(moduleName);
        } catch (Exception e) {
            LogUtils.warn("判断来源用例是否在两库失败, sourceCaseId: " + sourceCaseId, e);
            return false;
        }
    }

    /**
     * 根据「当前用例内容 vs 源用例」计算复用类型：仅标题不同为直接复用，其它内容有差异为适配复用。
     */
    private String computeReuseType(String currentName, String currentSteps, String currentTextDescription,
                                    String currentExpectedResult, String currentPrerequisite, String currentDescription,
                                    FunctionalCase sourceCase, FunctionalCaseBlob sourceBlob) {
        String sourceName = sourceCase != null ? sourceCase.getName() : "";
        String sSteps = blobBytesToString(sourceBlob != null ? sourceBlob.getSteps() : null);
        String sTextDesc = blobBytesToString(sourceBlob != null ? sourceBlob.getTextDescription() : null);
        String sExpected = blobBytesToString(sourceBlob != null ? sourceBlob.getExpectedResult() : null);
        String sPreq = blobBytesToString(sourceBlob != null ? sourceBlob.getPrerequisite() : null);
        String sDesc = blobBytesToString(sourceBlob != null ? sourceBlob.getDescription() : null);
        String cSteps = StringUtils.defaultString(currentSteps);
        String cTextDesc = StringUtils.defaultString(currentTextDescription);
        String cExpected = StringUtils.defaultString(currentExpectedResult);
        String cPreq = StringUtils.defaultString(currentPrerequisite);
        String cDesc = StringUtils.defaultString(currentDescription);
        boolean onlyNameDiff = StringUtils.equals(cSteps, sSteps)
                && StringUtils.equals(cTextDesc, sTextDesc)
                && StringUtils.equals(cExpected, sExpected)
                && StringUtils.equals(cPreq, sPreq)
                && StringUtils.equals(cDesc, sDesc)
                && !StringUtils.equals(StringUtils.defaultString(currentName), sourceName);
        String reuseType = onlyNameDiff ? CaseMetricsConstants.ReuseType.DIRECT_REUSE : CaseMetricsConstants.ReuseType.ADAPT_REUSE;
        return reuseType;
    }

    private static String blobBytesToString(byte[] bytes) {
        if (bytes == null) {
            return "";
        }
        try {
        ...
        if (CollectionUtils.isNotEmpty(ids)) {
            ...
            for (String s : ids) {
                ...
                Optional.ofNullable(functionalCase).ifPresent(functional -> {
                    ...
                    functional.setSourceCaseId(s);
                    if (isSourceFromTwoLibraries(s)) {
                        functional.setReuseType(CaseMetricsConstants.ReuseType.DIRECT_REUSE);
                    }
                ...
                if (CollectionUtils.isNotEmpty(caseAttachments)) {
                    ...
                    caseAttachments.forEach(attachment -> {
                        ...
                        // 如果是本地文件，需要复制实际文件
                        if (BooleanUtils.isTrue(attachment.getLocal())) {
                            try {
                                FileCopyRequest fileCopyRequest = new FileCopyRequest();
                                fileCopyRequest.setCopyFolder(DefaultRepositoryDir.getFunctionalCaseDir(request.getProjectId(), s) + "/" + attachment.getFileId());
                                fileCopyRequest.setCopyfileName(attachment.getFileName());
                                fileCopyRequest.setFileName(attachment.getFileName());
                                fileCopyRequest.setFolder(DefaultRepositoryDir.getFunctionalCaseDir(request.getProjectId(), id) + "/" + attachment.getFileId());
                                defaultRepository.copyFile(fileCopyRequest);
                            } catch (Exception e) {
                                LogUtils.error("复制用例附件文件失败，用例ID: " + id + ", 文件ID: " + attachment.getFileId() + ", 错误: " + e.getMessage(), e);
                            }
                        }
     ...
     */
    public void copyModuleWithCases(String sourceModuleId, String targetModuleId, String projectId, String userId, String organizationId) {
        // 1. 获取源模块信息
        FunctionalCaseModule sourceModule = functionalCaseModuleMapper.selectByPrimaryKey(sourceModuleId);
        if (sourceModule == null) {
            throw new MSException(Translator.get("case_module.not.exist"));
        }
        
        // 2. 递归创建模块结构并建立映射关系
        Map<String, String> moduleIdMapping = new HashMap<>(); // 源模块ID -> 新模块ID
        copyModuleStructure(sourceModule, targetModuleId, projectId, userId, moduleIdMapping);
        
        // 3. 将每个源模块下的用例复制到对应的新模块中
        for (Map.Entry<String, String> entry : moduleIdMapping.entrySet()) {
            String oldModuleId = entry.getKey();
            String newModuleId = entry.getValue();
            ...
            caseExample.createCriteria()
                    ...
                    .andModuleIdEqualTo(oldModuleId);
            List<FunctionalCase> cases = functionalCaseMapper.selectByExample(caseExample);
            
            if (CollectionUtils.isNotEmpty(cases)) {
                List<String> caseIds = cases.stream().map(FunctionalCase::getId).collect(Collectors.toList());
                FunctionalCaseBatchMoveRequest request = new FunctionalCaseBatchMoveRequest();
                request.setProjectId(projectId);
                request.setModuleId(newModuleId);
                request.setSelectIds(caseIds);
                request.setSelectAll(false);
                batchCopyFunctionalCase(request, userId, organizationId);
            }
        }
    }
     ...
     * @param targetParentId 目标父模块ID
     * @param projectId      项目ID
     * @param userId         用户ID
     * @param moduleIdMapping 模块ID映射（源模块ID -> 新模块ID）
     * @return 新创建的模块ID
     */
    private String copyModuleStructure(FunctionalCaseModule sourceModule, String targetParentId, String projectId, String userId,
                                       Map<String, String> moduleIdMapping) {
        // 1. 创建新模块
        ...
        createRequest.setName(sourceModule.getName());
        createRequest.setParentId(targetParentId);
        String newModuleId = functionalCaseModuleService.add(createRequest, userId);
        ...
        example.createCriteria().andParentIdEqualTo(sourceModule.getId());
        List<FunctionalCaseModule> children = functionalCaseModuleMapper.selectByExample(example);
        
        if (CollectionUtils.isNotEmpty(children)) {
            for (FunctionalCaseModule child : children) {
                copyModuleStructure(child, newModuleId, projectId, userId, moduleIdMapping);
            }
        }
     ...
     * @return 所有子模块ID列表
     */
    private List<String> getAllSubModuleIds(String moduleId) {
        List<String> allSubModuleIds = new ArrayList<>();
        List<String> currentLevelModuleIds = new ArrayList<>();
        currentLevelModuleIds.add(moduleId);
        
        while (CollectionUtils.isNotEmpty(currentLevelModuleIds)) {
            List<String> childrenIds = extFunctionalCaseModuleMapper.selectChildrenIdsByParentIds(currentLevelModuleIds);
            if (CollectionUtils.isNotEmpty(childrenIds)) {
                allSubModuleIds.addAll(childrenIds);
                currentLevelModuleIds = new ArrayList<>(childrenIds);
            } else {
                break;
            }
        }
        
        return allSubModuleIds;
    }

    private String getCopyName(String name, long oldNum, long newNum) {
        // 如果名称已经以 copy_ 开头，先去掉 copy_ 前缀，避免重复
        if (StringUtils.startsWith(name, "copy_")) {
            name = name.substring(5); // 去掉 "copy_" 前缀
        }
        // 如果名称以 _数字 结尾（可能是之前的ID），去掉这个后缀
        if (StringUtils.endsWith(name, "_" + oldNum)) {
            name = StringUtils.substringBeforeLast(name, "_" + oldNum);
        }
        // 只在前面添加 copy_ 前缀，不再添加ID后缀
        name = "copy_" + name;
        if (name.length() > 255) {
    ...
    public void saveImportData(List<FunctionalCaseExcelData> list, FunctionalCaseImportRequest request, List<BaseTreeNode> moduleTree, Map<String, TemplateCustomFieldDTO> customFieldsMap, Map<String, String> pathMap, SessionUser user, AtomicLong lastPos) {
        ...
        // 导入后异步触发 CS 计算，为新增用例写入 case_metrics_detail（含 source_case_id/REUSE），复用指标才能统计
        if (CollectionUtils.isNotEmpty(caseIds)) {
            caseCSCalculationService.batchCalculateMetricsDetailForCaseIdsAsync(new ArrayList<>(caseIds));
        }
    ...
    private void parseInsertDataToModule(FunctionalCaseExcelData functionalCaseExcelData, FunctionalCaseImportRequest request, String userId, Map<String, String> caseModulePathMap, TemplateDTO defaultTemplateDTO, Long nextOrder,
        ...
        functionalCase.setCreateTime(System.currentTimeMillis());
        functionalCase.setUpdateTime(System.currentTimeMillis());
        // 从两库导出后再导入的，全部算适配复用（不按内容 diff 区分直接/适配）
        if (StringUtils.isNotBlank(functionalCaseExcelData.getSourceCaseId())) {
            String sourceId = functionalCaseExcelData.getSourceCaseId().trim();
            functionalCase.setSourceCaseId(sourceId);
            if (isSourceFromTwoLibraries(sourceId)) {
                functionalCase.setReuseType(CaseMetricsConstants.ReuseType.ADAPT_REUSE);
            }
        }
