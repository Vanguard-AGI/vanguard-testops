...
import io.vanguard.testops.plan.dto.TestPlanExecuteHisDTO;
import io.vanguard.testops.plan.dto.TestPlanResourceExecResultDTO;
import io.vanguard.testops.plan.dto.request.*;
import io.vanguard.testops.plan.dto.response.TestPlanCoverageDTO;
import io.vanguard.testops.plan.dto.response.TestPlanDetailResponse;
import io.vanguard.testops.plan.dto.response.TestPlanOperationResponse;
import io.vanguard.testops.plan.dto.response.TestPlanStatisticsResponse;
import io.vanguard.testops.plan.enums.ExecuteMethod;
import io.vanguard.testops.plan.job.TestPlanScheduleJob;
import io.vanguard.testops.plan.mapper.*;
import io.vanguard.testops.project.request.ProjectApplicationRequest;
...
import io.vanguard.testops.system.domain.ScheduleExample;
...
import io.vanguard.testops.system.mapper.ScheduleMapper;
import io.vanguard.testops.system.mapper.TestPlanModuleMapper;
...
import io.vanguard.testops.system.support.batch.BatchProcessUtils;
...
public class TestPlanService extends TestPlanBaseUtilsService {
    ...
    public TestPlan add(TestPlanCreateRequest testPlanCreateRequest, String operator, String requestUrl, String requestMethod) {
        ...
        // 测试计划创建后，自动计算指标（仅对 TEST_PLAN 类型）
        if (StringUtils.equalsIgnoreCase(testPlan.getType(), TestPlanConstants.TEST_PLAN_TYPE_PLAN)) {
            try {
                testPlanMetricsService.calculateAndSaveMetricsAsync(testPlan.getId());
                LogUtils.info("测试计划创建后自动计算指标任务已提交，测试计划ID: {}", testPlan.getId());
            } catch (Exception e) {
                LogUtils.error("测试计划创建后自动计算指标失败，测试计划ID: " + testPlan.getId(), e);
                // 指标计算失败不影响测试计划创建流程，只记录日志
            }
        }
    ...
    private TestPlan savePlanDTO(TestPlanCreateRequest createOrCopyRequest, String operator) {
        ...
        if (!StringUtils.equals(createTestPlan.getGroupId(), TestPlanConstants.TEST_PLAN_DEFAULT_GROUP_ID)) {
            ...
            // 测试计划创建到测试计划组时，继承组的需求
            if (StringUtils.isNotBlank(groupPlan.getFeishuStoryId())) {
                createTestPlan.setFeishuStoryId(groupPlan.getFeishuStoryId());
            }
        }
        ...
        // 创建测试计划时若带了需求，回填该需求下 projectId 为空的飞书缺陷
        if (StringUtils.isNotBlank(createTestPlan.getFeishuStoryId())) {
            backfillBugProjectByStory(createTestPlan.getFeishuStoryId(), createTestPlan.getProjectId());
        }
    ...
    public TestPlan update(TestPlanUpdateRequest request, String userId, String requestUrl, String requestMethod) {
        ...
        updateTestPlan.setFeishuStoryId(request.getFeishuStoryId());
        
        // 判断是否为测试计划组，如果是则同步更新组内所有测试计划的需求
        boolean isGroup = StringUtils.equalsIgnoreCase(originalTestPlan.getType(), TestPlanConstants.TEST_PLAN_TYPE_GROUP);
        if (isGroup && request.getFeishuStoryId() != null) {
            syncGroupStoryIdToPlans(request.getId(), request.getFeishuStoryId());
        }

        // 需求关联/变更时，回填该需求下 projectId 为空的飞书缺陷
        if (StringUtils.isNotBlank(request.getFeishuStoryId())) {
            backfillBugProjectByStory(request.getFeishuStoryId(), originalTestPlan.getProjectId());
        }
        ...
        if (StringUtils.isNotBlank(request.getGroupId()) && !StringUtils.equalsIgnoreCase(request.getGroupId(), TestPlanConstants.TEST_PLAN_DEFAULT_GROUP_ID)) {
            if (!StringUtils.equalsIgnoreCase(originalTestPlan.getGroupId(), request.getGroupId())) {
                ...
                // 测试计划移入测试计划组时，继承组的需求
                if (StringUtils.isNotBlank(testPlanGroup.getFeishuStoryId())) {
                    updateTestPlan.setFeishuStoryId(testPlanGroup.getFeishuStoryId());
                }
            }
        } else {
        ...
        // 测试计划更新后，自动重新计算指标（仅对 TEST_PLAN 类型）
        if (StringUtils.equalsIgnoreCase(testPlan.getType(), TestPlanConstants.TEST_PLAN_TYPE_PLAN)) {
            try {
                testPlanMetricsService.calculateAndSaveMetricsAsync(testPlan.getId());
                LogUtils.info("测试计划更新后自动计算指标任务已提交，测试计划ID: {}", testPlan.getId());
            } catch (Exception e) {
                LogUtils.error("测试计划更新后自动计算指标失败，测试计划ID: " + testPlan.getId(), e);
                // 指标计算失败不影响测试计划更新流程，只记录日志
            }
        }
    ...
    public TestPlanDetailResponse detail(String id, String userId) {
        ...
        response.setFeishuStoryId(testPlan.getFeishuStoryId());
        if (StringUtils.equalsIgnoreCase(testPlan.getType(), TestPlanConstants.TEST_PLAN_TYPE_PLAN)) {
            //计划的 其他参数
    ...
    /**
     * 同步测试计划组的需求到组内所有测试计划
     * 当测试计划组关联/更新需求时，组内所有测试计划自动继承该需求
     * 
     * @param groupId 测试计划组ID
     * @param feishuStoryId 飞书需求ID
     */
    public void syncGroupStoryIdToPlans(String groupId, String feishuStoryId) {
        ...
        List<TestPlan> plansInGroup = testPlanMapper.selectByExample(example);
        
        if (CollectionUtils.isEmpty(plansInGroup)) {
            return;
        }
        
        // 批量更新组内测试计划的需求
        TestPlan updateRecord = new TestPlan();
        updateRecord.setFeishuStoryId(feishuStoryId);
        updateRecord.setUpdateTime(System.currentTimeMillis());
        testPlanMapper.updateByExampleSelective(updateRecord, example);
    }

    /**
     * 当测试计划关联/修改需求时，回填该需求下所有飞书缺陷的 projectId。
     * 链路：feishuStoryId → bug.platform_bug_id（飞书缺陷关联同一 Story）→ 更新 bug.project_id
     */
    public void backfillBugProjectByStory(String feishuStoryId, String projectId) {
        if (StringUtils.isBlank(feishuStoryId) || StringUtils.isBlank(projectId)) {
            return;
        }
        BugExample example = new BugExample();
        example.createCriteria()
                .andPlatformEqualTo("FEISHU")
                .andProjectIdEqualTo("");
        ...
        if (updated > 0) {
            LogUtils.info("[TestPlan] 需求 {} 关联测试计划，回填 {} 条飞书缺陷的 projectId={}", feishuStoryId, updated, projectId);
        }
    }
}
