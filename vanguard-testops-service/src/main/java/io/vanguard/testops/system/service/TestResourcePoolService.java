...
import io.vanguard.testops.sdk.constants.ResourcePoolTypeEnum;
...
import io.vanguard.testops.system.dto.pool.*;
...
public class TestResourcePoolService {
    ...
    public void addTestResourcePool(TestResourcePoolDTO testResourcePool) {
        checkTestResourcePool(testResourcePool);
        testResourcePool.setId(IDGenerator.nextStr());
    ...
    public boolean isPoolAvailable(String resourcePoolId) {
        TestResourcePool testResourcePool = testResourcePoolMapper.selectByPrimaryKey(resourcePoolId);
        return testResourcePool != null
                && BooleanUtils.isTrue(testResourcePool.getEnable())
                && BooleanUtils.isFalse(testResourcePool.getDeleted());
    }
    ...
    public LogDTO deleteLog(TestResourcePoolRequest request) {
        TestResourcePool pool = testResourcePoolMapper.selectByPrimaryKey(request.getId());
        if (pool != null) {
            LogDTO dto = new LogDTO(
                    OperationLogConstants.SYSTEM,
