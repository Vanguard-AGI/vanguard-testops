            ...
            steps {
                configFileProvider([configFile(fileId: 'metersphere-maven', targetLocation: 'settings.xml')]) {
                    sh '''#!/bin/bash -xe
                        ...
                        ./mvnw clean deploy -T 1C -B -Drevision=${REVISION} -DskipTests -DskipAntRunForJenkins -pl !start --settings ./settings.xml
                    '''
                }
            }
        }
            ...
            steps {
                configFileProvider([configFile(fileId: 'metersphere-maven', targetLocation: 'settings.xml')]) {
                    sh '''#!/bin/bash -xe
                        ...
                        mkdir -p start/target/dependency && (cd start/target/dependency && jar -xf ../*.jar);
                    '''
                }
            }
        }
        stage('Community build & push') {
            ...
            steps {
                configFileProvider([configFile(fileId: 'metersphere-maven', targetLocation: 'settings.xml')]) {
                    ...
                    do
                        cp -rf $LOCAL_REPOSITORY/io/metersphere/$library/${REVISION}/$library-${REVISION}.jar start/target/dependency/BOOT-INF/lib/
                    done
