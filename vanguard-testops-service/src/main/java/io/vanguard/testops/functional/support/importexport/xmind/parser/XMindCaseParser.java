package io.vanguard.testops.functional.support.importexport.xmind.parser;

import io.vanguard.testops.functional.constants.FunctionalCaseTypeConstants;
import io.vanguard.testops.functional.dto.FunctionalCaseStepDTO;
...
import io.vanguard.testops.functional.support.importexport.xmind.pojo.Attached;
import io.vanguard.testops.functional.support.importexport.xmind.pojo.JsonRootBean;
import io.vanguard.testops.functional.support.importexport.xmind.support.DetailUtil;
...
public class XMindCaseParser {
    ...
    public boolean validate(FunctionalCaseExcelData data) {
        ...
        //必填字段校验
        validate = validateRequiredFields(data);
        return validate;
    }
     ...
     * 校验必填字段
     */
    private boolean validateRequiredFields(FunctionalCaseExcelData data) {
        if (StringUtils.isBlank(data.getPrerequisite())) {
            process.add(data.getName(), Translator.get("preconditions_optional"));
            return false;
        }
        if (StringUtils.equalsIgnoreCase(data.getCaseEditType(), FunctionalCaseTypeConstants.CaseEditType.STEP.name())) {
            if (!hasValidStepDescription(data)) {
                process.add(data.getName(), Translator.get("test_case_step_required"));
                return false;
            }
            if (!hasAllStepsWithExpectedResult(data)) {
                process.add(data.getName(), Translator.get("test_case_expected_result_required"));
                return false;
            }
        }
        return true;
    ...
    private boolean hasValidStepDescription(FunctionalCaseExcelData data) {
        if (StringUtils.isBlank(data.getSteps())) {
            return false;
        }
        try {
            List<Map> steps = JSON.parseArray(data.getSteps());
            if (CollectionUtils.isEmpty(steps)) {
                return false;
            }
            for (Map step : steps) {
                Object desc = step.get("desc");
                if (desc != null && StringUtils.isNotBlank(desc.toString())) {
                    return true;
                }
            }
            return false;
    ...
    private boolean hasAllStepsWithExpectedResult(FunctionalCaseExcelData data) {
        ...
        try {
            ...
            for (Map step : steps) {
                ...
                boolean hasDesc = desc != null && StringUtils.isNotBlank(desc.toString());
                boolean hasResult = result != null && StringUtils.isNotBlank(result.toString());
                if (hasDesc && !hasResult) {
                    return false;
                }
            }
            return true;
