...
import io.vanguard.testops.api.service.ApiEnvironmentService;
...
import io.vanguard.testops.sdk.util.WebSocketUtils;
    ...
    public void debugConsume(ConsumerRecord<?, String> record) {
        try {
            ...
            if (ObjectUtils.isNotEmpty(record.value()) && WebSocketUtils.has(record.key().toString())) {
                ...
                // 处理环境变量
                if (dto.getTaskResult() instanceof List) {
                    List<?> environmentVariables = (List<?>) dto.getTaskResult();
                    LogUtils.info("{} 处理环境变量", record.key());
                    apiEnvironmentService.parseEnvironment(environmentVariables.stream()
