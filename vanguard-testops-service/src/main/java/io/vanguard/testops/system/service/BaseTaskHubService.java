...
import io.vanguard.testops.api.domain.ApiReportRelateTask;
import io.vanguard.testops.api.domain.ApiReportRelateTaskExample;
import io.vanguard.testops.api.mapper.ApiReportRelateTaskMapper;
...
import io.vanguard.testops.system.dto.BatchExecTaskReportDTO;
...
import io.vanguard.testops.system.dto.request.BatchExecTaskPageRequest;
...
import io.vanguard.testops.system.dto.taskhub.*;
import io.vanguard.testops.system.dto.taskhub.request.ScheduleRequest;
import io.vanguard.testops.system.dto.taskhub.request.TaskHubItemBatchRequest;
import io.vanguard.testops.system.dto.taskhub.request.TaskHubItemRequest;
import io.vanguard.testops.system.dto.taskhub.response.TaskStatisticsResponse;
...
import io.vanguard.testops.system.runtime.schedule.ApiScheduleNoticeService;
...
import io.vanguard.testops.system.support.taskrunner.TaskRunnerClient;
    ...
    private boolean determineStatus(String key, List<ExecTaskItem> items) {
        try {
            ...
            if (split.length == 2) {
                ...
                return EngineBridge.validateNamespaceExists(testResourceDTO);
            }
        } catch (Exception e) {
    ...
    private void handleK8STask(List<String> taskIds, TestResourcePoolReturnDTO testResourcePoolDTO) {
        SubListUtils.dealForSubList(taskIds, 100, subList -> {
            try {
                ...
                EngineBridge.stopApiTask(subList, testResourceDTO);
            } catch (Exception e) {
                LogUtils.error(e);
            }
        });
    ...
    private void handleK8STaskItem(List<String> itemIds, TestResourcePoolReturnDTO testResourcePoolDTO) {
        SubListUtils.dealForSubList(itemIds, 100, subList -> {
            try {
                TestResourceDTO testResourceDTO = new TestResourceDTO();
                BeanUtils.copyBean(testResourceDTO, testResourcePoolDTO.getTestResourceReturnDTO());
                EngineBridge.stopApiTaskItem(subList, testResourceDTO);
            } catch (Exception e) {
                LogUtils.error(e);
            }
        });
