...
import io.vanguard.testops.functional.domain.FunctionalCaseAttachmentExample;
import io.vanguard.testops.functional.dto.FunctionalCaseAttachmentDTO;
import io.vanguard.testops.functional.dto.FunctionalCaseDetailDTO;
import io.vanguard.testops.functional.mapper.FunctionalCaseAttachmentMapper;
import io.vanguard.testops.functional.request.FunctionalCaseAssociationFileRequest;
import io.vanguard.testops.functional.request.FunctionalCaseDeleteFileRequest;
...
public class FunctionalCaseAttachmentService {
    ...
    public Map<String, List<FunctionalCaseAttachment>> getAttachmentByCaseIds(List<String> ids) {
        List<String> sources = List.of(CaseFileSourceType.ATTACHMENT.toString(), CaseFileSourceType.CASE_DETAIL.toString(), CaseFileSourceType.CASE_COMMENT.toString());
        FunctionalCaseAttachmentExample example = new FunctionalCaseAttachmentExample();
    ...
    public void uploadOssFile(String caseId, String projectId, List<String> uploadFileIds, String userId, String fileSource) {
    ...
    public ResponseEntity<byte[]> downloadImgById(String projectId, String fileId, boolean compressed) {
        ...
        if (CollectionUtils.isEmpty(caseAttachments)) {
            ...
            //在正式目录获取
            // 如果有多条记录，优先选择 CASE_DETAIL 类型的记录（备注中的图片通常是这个类型）
            FunctionalCaseAttachment attachment = null;
            if (caseAttachments.size() > 1) {
                attachment = caseAttachments.stream()
                    .filter(a -> CaseFileSourceType.CASE_DETAIL.toString().equals(a.getFileSource()))
                    .findFirst()
                    .orElse(caseAttachments.getFirst());
            } else {
                attachment = caseAttachments.getFirst();
            }
            ...
            try {
                ...
                // 如果下载失败，且有多条记录，尝试其他记录
                if (caseAttachments.size() > 1) {
                    for (FunctionalCaseAttachment otherAttachment : caseAttachments) {
                        if (otherAttachment.getId().equals(attachment.getId())) {
                            continue; // 跳过已经尝试过的
                        }
                        try {
                            fileRequest.setFileName(otherAttachment.getFileName());
                            if (compressed) {
                                fileRequest.setFolder(DefaultRepositoryDir.getFunctionalCasePreviewDir(projectId, otherAttachment.getCaseId()) + "/" + otherAttachment.getFileId());
                            } else {
                                fileRequest.setFolder(DefaultRepositoryDir.getFunctionalCaseDir(projectId, otherAttachment.getCaseId()) + "/" + otherAttachment.getFileId());
                            }
                            bytes = fileService.download(fileRequest);
                            fileName = otherAttachment.getFileName();
                            break; // 成功找到文件，跳出循环
                        } catch (Exception ex) {
                            // 继续尝试下一条记录
                            continue;
